...
const ProductDetails = ({ selectedProduct, addToCart, editListing, leaveReview, deleteListing, modalVisibility, toggleModalVisibility, productFormState, setProductFormState, editing, putProduct }) => {
    ...
    <div className="container product-details__container">
      ...
      {!selectedProduct ? (
        ...
        <div className="product-details">
          ...
          <h2 className="product-details__price">${selectedProduct.price.toLocaleString('en-US')}</h2>
          ...
          <div className="button-container">
            <button className="btn" onClick={
              selectedProduct.seller_id !== user.id ? (()=>addToCart(selectedProduct.id)) : (()=>editListing(selectedProduct.id))
            }>
              {
              selectedProduct.seller_id !== user.id ? ('add to cart') : ('edit listing')
              }
            </button>
            <button className="btn btn-alt" onClick={
              selectedProduct.seller_id !== user.id ? (()=>leaveReview(selectedProduct.id)) : (()=>deleteListing(selectedProduct.id))
            }>
              {
              selectedProduct.seller_id !== user.id ? ('leave review') : ('delete listing')
              }
            </button>
            </div>
