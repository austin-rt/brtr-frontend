...
const ProductList = ({ productsFeed, chooseProduct, addToCart, editListing, leaveReview, deleteListing }) => {
        ...
        <div className="product__item-card" key={product.id}>
          ...
          <div className="product__hero-info">
            ...
            <div className="product__hero-info-grid">
              <div className="product__info-left-column">
                <h3 className="product__price">${product.price.toLocaleString('en-US')}</h3>
              </div>
          ...
          <div className="button-container">
            <button className="btn" onClick={
              user ? (
              product.seller_id !== user?.id ? (()=>addToCart(product.id)) : (()=>editListing(product.id))
              ) : (
                (()=>navigate('/login'))
              )
            }>
              {
                user ? (
                  product.seller_id !== user.id ? ('add to cart') : ('edit listing')
                ) : (
                  ('add to cart')
                )
              }
            ...
            <button className="btn btn-alt" onClick={
              user ? (
                product.seller_id !== user.id ? (()=>leaveReview(product.id)) : (()=>deleteListing(product.id))
              ) : (
                (()=>navigate('/login'))
              )
            }>
              {
                user ? (
              product.seller_id !== user.id ? ('leave review') : ('delete listing')
                ) : (
                  ('leave review')
                )
              }
